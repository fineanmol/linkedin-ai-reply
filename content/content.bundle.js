...
`;Ce=class{constructor(){this.host=null,this.shadow=null}ensureLauncher(){if(document.getElementById(Ze))return;this.host=document.createElement("div"),this.host.id=Ze,this.shadow=this.host.attachShadow({mode:"open"});let e=document.createElement("style");e.textContent=At,this.shadow.appendChild(e),this.root=document.createElement("div"),this.shadow.appendChild(this.root),document.body.appendChild(this.host),this.renderLauncher()}async renderLauncher(){let{queue:e=[]}=await m(p.GET_QUEUE),t=e.filter(n=>n.status!=="skipped"&&n.status!=="done").length;if(_e){this.renderPanel(e);return}this.root.innerHTML=`
      <button class="launcher" id="q-launch">
        ${et(20,"#ffffff","#3b9dbf")} <span>Engagement</span> <span class="badge">${t}</span>
      </button>`,this.root.querySelector("#q-launch").onclick=()=>{_e=!0,this.renderLauncher()}}async renderPanel(){let e=Je,t=e==="connections"?await this._connectionsBody():await this._commentsBody();this.root.innerHTML=`
      <div class="backdrop" id="q-backdrop">
        <div class="panel" role="dialog" aria-label="LinkedIn Assistant">
          <div class="head">
            <h3>${et(18,"#5cc3e8","#0c1a22")} LinkedIn Assistant</h3>
      ...
      </div>`;let n=()=>{_e=!1,this.renderLauncher()};this.root.querySelector("#q-min").onclick=n,this.root.querySelector("#q-backdrop").onclick=r=>{r.target.id==="q-backdrop"&&n()},this.root.querySelectorAll(".tab").forEach(r=>r.onclick=()=>{Je=r.dataset.tab,this.renderPanel()}),e==="connections"?this._wireConnections():this._wireComments()}async _commentsBody(){let{queue:e=[]}=await m(p.GET_QUEUE),{counts:t={today:0,week:0,total:0}}=await m(p.GET_COMMENTS_LOG),r=e.filter(i=>i.status!=="skipped").sort((i,a)=>(i.status==="done"?1:0)-(a.status==="done"?1:0)).map(i=>{let a=i.relevance!=null?`${Math.round(i.relevance*100)}%`:"",s=i.status==="done",l=i.status==="copied";return`
...
`});var Y,at,it,te,st,lt,ct,k,oe=E(()=>{Y=`
...
`,k={question:`
  ...
  `}});function pt(o){return String(o).replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;")}function ke(o,e,t){let n=dt[o]||dt.general,r=e||{},i=r.llmBackend||"gemini",a=i==="ollama"?r.ollamaModel||"gemma2:2b":r.geminiModel||"gemini-flash-latest",s=a.replace("gemini-","Gemini ").replace("-flash"," Flash").replace("-pro"," Pro"),l=`background: ${n.bg}; color: ${n.color}; border: 1px solid ${n.color}33;`,c=o==="feedback",u=c?"#314855":n.color,y=c?`background: ${n.bg}; color: ${u}; border: 1px solid rgba(49, 72, 85, 0.2);`:l;return`
    <div class="panel-header">
      <div class="panel-title">
        ${Y}
        AI Reply
      </div>
      <span class="intent-badge" style="${y}">
        ${n.icon}
      ...
      <div class="model-switcher" id="liar-model-switcher">
        <div class="model-pill" id="liar-model-pill" title="Switch model">
          ...
          <span>${s}</span>
          <span style="margin-left: 6px; display: flex; align-items: center;">${ct}</span>
        </div>
  ...
  `}function Pt(o,e,t){let n=gt.map(i=>`
  ...
  `).join(""),r=t&&t.length>0?t.map(i=>{let a=Pe.find(l=>l.id===i),s=a?a.label.split("\u2014")[0].trim():i;return`
    ...
    ${r}
  `}function ut(o,e,t){return`
    ${ke(o,e,t)}
    <div class="loading-state">
      <div class="spinner"></div>
      <span class="loading-text">Analyzing style and drafting suggestion\u2026</span>
    </div>
  `}function mt(o,e,t,n,r,i,a){return`
    ${ke(o,e,t)}
    <textarea class="reply-textarea" id="liar-textarea" spellcheck="true" placeholder="AI suggestion draft...">${pt(n)}</textarea>
    <div class="meta-row">
  ...
  `}var dt,ht=E(()=>{oe();Ne();C();dt={question:{label:"Question",color:"#e95f5c",icon:k.question,bg:"rgba(233, 95, 92, 0.1)"},appreciation:{label:"Appreciation",color:"#79ceb8",icon:k.appreciation,bg:"rgba(121, 206, 184, 0.1)"},feedback:{label:"Feedback",color:"#ffdb00",icon:k.feedback,bg:"rgba(255, 219, 0, 0.15)"},criticism:{label:"Criticism",color:"#e95f5c",icon:k.criticism,bg:"rgba(233, 95, 92, 0.1)"},technical:{label:"Technical",color:"#5cc3e8",icon:k.technical,bg:"rgba(92, 195, 232, 0.1)"},networking:{label:"Networking",color:"#5cc3e8",icon:k.networking,bg:"rgba(92, 195, 232, 0.1)"},general:{label:"General",color:"#314855",icon:k.general,bg:"rgba(49, 72, 85, 0.08)"}}});var gt,ne,Ne=E(()=>{C();v();_();rt();ht();oe();gt=[{id:"gemini-flash-latest",label:"Gemini Flash (latest) \u2014 recommended"},{id:"gemini-3.5-flash",label:"Gemini 3.5 Flash"},{id:"gemini-3.1-flash-lite",label:"Gemini 3.1 Flash-Lite \u2014 fastest/cheapest"},{id:"gemini-3.1-pro-preview",label:"Gemini 3.1 Pro \u2014 advanced reasoning"}],ne=class{constructor(e){this.opts=e,this.shadowHost=null,this.shadow=null,this.currentReply="",this.backend="",this.model="",this._generationActive=!1,this._settings=null,this._ollamaModels=[]}mount(e){let t=`liar-panel-${this.opts.commentId}`;document.getElementById(t)?.remove(),this.shadowHost=document.createElement("div"),this.shadowHost.id=t,this.shadowHost.className="liar-shadow-host",this.shadowHost.style.cssText="display:block;width:100%;",this.shadow=this.shadowHost.attachShadow({mode:"closed"});let n=document.createElement("style");n.textContent=nt,this.shadow.appendChild(n),this._container=document.createElement("div"),this._container.className="panel",this.shadow.appendChild(this._container),e.parentNode?.insertBefore(this.shadowHost,e.nextSibling),this.shadowHost.isConnected||e.after(this.shadowHost),this._loadSettingsAndGenerate()}async _loadSettingsAndGenerate(){try{let[e,t]=await Promise.all([chrome.runtime.sendMessage({type:p.GET_SETTINGS}),chrome.runtime.sendMessage({type:p.GET_OLLAMA_MODELS}).catch(()=>({models:[]}))]);this._settings=e||{},this._ollamaModels=t?.models||[]}catch{this._settings={},this._ollamaModels=[]}this._renderLoading(),this._generate()}unmount(){this._cancelInflight(),this._closeDropdownListener&&document.removeEventListener("click",this._closeDropdownListener,{capture:!0}),this.shadowHost?.remove()}_renderLoading(){this._container.innerHTML=ut(this.opts.intent,this._settings,this._ollamaModels),this._bindClose()}_renderReply(e,t,n){this.currentReply=e,this.backend=t,this.model=n;let r=e.split(/\s+/).filter(Boolean).length;this._container.innerHTML=mt(this.opts.intent,this._settings,this._ollamaModels,e,t,n,r),this._bindClose(),this._bindActions()}_renderError(e){let t=e.toLowerCase().includes("ollama")||e.includes("localhost"),n=t&&(e.includes("403")||e.toLowerCase().includes("forbidden")||e.toLowerCase().includes("cors")),r="";n?r=`
...
`).filter(u=>!["Like","Reply","React","See more","See less","\u2022"].includes(u.trim())).join(" ").trim();c.length>=3&&(n.text=c)}d.log("handleButtonClick: sending to LLM \u2192",{commentId:n.id,author:n.authorName,text:n.text.slice(0,120),postContentLength:r.length});let{intent:i}=Te(n.text),a=n.id;if(O.has(a)){O.get(a).unmount(),O.delete(a),o.classList.remove("active");return}o.classList.add("active");let s=new ne({commentId:n.id,commentText:n.text,authorName:n.authorName,postContent:r,intent:i,onClose:()=>{O.delete(a),o.classList.remove("active")},onApprove:({text:l})=>{d.log("UIInjector: reply approved for comment",a),o.innerHTML=`
      ...
      `,o.classList.add("approved"),setTimeout(()=>{o.innerHTML=`
          ${Y}
          <span>AI Reply</span>
        `,o.classList.remove("approved")},3e3)}});O.set(a,s),s.mount(e)}function Ie(){for(let o of O.values())o.unmount();O.clear()}var bt,O,xt=E(()=>{ot();v();ye();Ne();C();_();oe();bt=new WeakSet,O=new Map});var qt=vt(()=>{be();ye();xe();Ee();we();tt();xt();v();ge();_();C();var re=!0,H=null,Et=location.href,F=null,ae=new Set;async function wt(){try{console.log("%c[LIAR] LinkedIn AI Reply Assistant content script loaded v1.0.5","color: #6366f1; font-weight: bold;");let o=await Qe();if(re=o.enabled!==!1,ce(o.debugMode),!re){d.log("Extension is disabled.");return}await V();let{name:e,profilePath:t}=U();console.log("[LIAR] Loaded identity:",{name:e,profilePath:t}),e&&chrome.runtime.sendMessage({type:"SAVE_IDENTITY",payload:{name:e,profileUrl:t}}).catch(()=>{}),setTimeout(ie,1e3),setTimeout(()=>ve(),1200),Ct(),Dt(),setInterval(async()=>{let{name:n}=U();(!n||n==="Me")&&await V()},5e3)}catch(o){d.error("CRITICAL ERROR DURING INIT:",o)}}function ie(){if(!re)return;for(let n of ae)n.isConnected||ae.delete(n);let o=Ke(document),e=0;for(let n of o)try{let r=W(n.element)||document,i=K(r);yt(n.element,i),e++}catch(r){console.warn("[LIAR] comment processing threw:",r)}let t=document.querySelectorAll(".liar-ai-reply-btn").length;console.log(`%c[LIAR] scan: comments=${e} buttons=${t}`,"color:#0a66c2;font-weight:bold"),$t(e)}var Le=!1;function $t(o){if(o>0){Le=!1;return}let e=!!document.querySelector('[data-testid="ui-core-tiptap-text-editor-wrapper"], [contenteditable="true"][role="textbox"], [aria-label*="comment" i][contenteditable]'),t=!!document.querySelector('[componentkey^="comment-commentary_"]');(e||t)&&!Le&&(Le=!0,console.warn("%c[LIAR] \u26A0 Detection health: comment UI is present but 0 comments were detected. LinkedIn likely changed its DOM. Update the DETECTION anchors in utils/constants.js. (This warning fires once per page.)","color:#e6a860;font-weight:bold"))}function Ct(){H&&H.disconnect(),H=new MutationObserver(()=>{F&&clearTimeout(F),F=setTimeout(ie,100)}),H.observe(document.body,{childList:!0,subtree:!0}),d.log("MutationObserver started (debounced)")}function Dt(){let o=history.pushState.bind(history);history.pushState=(...e)=>{o(...e),_t()},window.addEventListener("popstate",_t)}async function _t(){let o=location.href;if(o===Et)return;Et=o,d.log("Navigation detected \u2192",o),Ie(),ae.clear(),setTimeout(()=>ve(),800),await V();let{name:e,profilePath:t}=U();e&&chrome.runtime.sendMessage({type:"SAVE_IDENTITY",payload:{name:e,profileUrl:t}}).catch(()=>{}),setTimeout(ie,1500)}chrome.runtime.onMessage.addListener((o,e,t)=>{if(o.type==="SETTINGS_CHANGED"){let{enabled:n,debugMode:r}=o.payload||{};typeof n=="boolean"&&(re=n,n?(ie(),Ct()):(Ie(),H?.disconnect(),F&&clearTimeout(F),ae.clear())),typeof r=="boolean"&&ce(r);return}if(o.type===p.REQUEST_BUILD_QUEUE)return(async()=>{try{let n=Z()?J(document):X(document);if(!n.length){let i=/linkedin\.com/.test(location.href);t({ok:!1,added:0,reason:i?"no-posts-found":"not-on-feed"});return}let r=await chrome.runtime.sendMessage({type:p.BUILD_QUEUE,payload:{posts:n}});console.log(`%c[LIAR] queue build: scanned=${r?.scanned} added=${r?.added}`,"color:#0a66c2;font-weight:bold"),t({ok:!0,...r})}catch(n){console.warn("[LIAR] build queue failed:",n),t({ok:!1,error:n.message})}})(),!0;if(o.type===p.REQUEST_SCAN_CONNECTIONS)return(async()=>{try{if(!Xe()){t({ok:!1,reason:"not-on-connections"});return}let n=ee(document);if(!n.length){t({ok:!1,reason:"no-connections-found"});return}let r=await chrome.runtime.sendMessage({type:p.ADD_CONNECTIONS,payload:{connections:n}});console.log(`%c[LIAR] connections scan: found=${n.length} added=${r?.added}`,"color:#0a66c2;font-weight:bold"),t({ok:!0,found:n.length,...r})}catch(n){console.warn("[LIAR] scan connections failed:",n),t({ok:!1,error:n.message})}})(),!0});document.readyState==="loading"?document.addEventListener("DOMContentLoaded",wt):wt()});qt();})();
